...
const bindings = {
  questionTopicClass: "sc-cSrtSx kguYDO",
  questionSkipperClass: "sc-cSrtSx kguYDO",
  isIncorrectClass: "sc-gCxsRJ cVzVHe\")"
}
...
function findButtonWithName(name) {
  ...
  for (const elem of document.getElementsByClassName(bindings.questionTopicClass)) {
    if (cnt == 0) {
      cnt++;
      continue;
    }
...
export async function ripClassic(msgApi) {
  console.log("RipClassicMode x GimTk");
